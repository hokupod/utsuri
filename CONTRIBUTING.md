...
2. Use the pinned Nix Bun to generate or repair a lockfile that every supported Bun can install with `--frozen-lockfile`. Renovate's Bun constraint matches that oldest supported writer. Automatic updates of this tool constraint are disabled; change it together with the Nix compatibility policy. `bunfig.toml` applies the existing three-day release age to newly resolved direct and transitive dependencies. For incompatible lock formats, preserve the reviewed lockfile outside the checkout, remove it, and run `node scripts/safe-chain.mjs bun install --lockfile-only --ignore-scripts` inside `nix develop`; then perform a frozen install. Do not hand-edit `lockfileVersion` or drop the compatibility check. Re-review all changed dependency versions and release ages if lock generation changes the dependency set.
