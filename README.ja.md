...
publisherは`hokupod`、npm maintainerは`hokupod-npm`、公開方式はexact tarballを使う手動の`v0.1.0` bootstrap後にGitHub Actions trusted publishingへ移行し、SPDX licenseは`AGPL-3.0-or-later`です。sourceは`v0.1.0` release-readyですが、5つのnpm packageとaggregate Pluginは未公開です。candidate生成はregistryへ書き込みません。tag workflowは、保護された`release` environment、exact release-asset integrity、公開packageのnative smoke、全asset upload成功後にのみ公開されるdraft GitHub Releaseを要求します。trusted publishingでは新規packageを作成できないため、認可済みmaintainerが5つのexactな初回versionを手動公開します。この書き込みにはGitHub Actions OIDC provenanceが付かないため、release guideは非secretのsource、candidate、tarball、publisher、時刻、registry integrity証跡の保持を要求します。以後の未公開versionには保護されたtrusted-publisher経路を使います。repository変更だけでtag作成やartifact公開は行われません。
