...
publisher 为 `hokupod`，npm maintainer 为 `hokupod-npm`，发布方式是在使用 exact tarball 手动完成 `v0.1.0` bootstrap 后改用 GitHub Actions trusted publishing，SPDX license 为 `AGPL-3.0-or-later`。source 已为 `v0.1.0` release-ready，但五个 npm package 与 aggregate Plugin 仍未发布。candidate 生成不会写入 registry。tag workflow 要求受保护的 `release` environment、exact release-asset integrity、已发布 package 的 native smoke，并仅在全部 asset upload 成功后公开 draft GitHub Release。由于 trusted publishing 无法创建新 package，获授权的 maintainer 必须手动发布五个 exact 初始版本；这些写入不带 GitHub Actions OIDC provenance，release guide 要求保留不含 secret 的 source、candidate、tarball、publisher、时间与 registry integrity 证据。后续缺失版本使用受保护的 trusted-publisher 路径。仅修改 repository 不会创建 tag 或发布 artifact。
