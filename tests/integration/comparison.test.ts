...
import { captureConfig, freePort, repositoryRoot, stopFixtureServer } from "./capture-helpers";
...
describe("comparison runtime", () => {
  test("produces deterministic pixel regions without declaring a pixel-only regression", async () => {
  ...
  test("classifies a new mobile overflow with screenshot evidence", async () => {
  ...
  test("retains a focused-control style regression as measured evidence", async () => {
  ...
  test("classifies an icon button accessible-name removal as new", async () => {
  ...
  test("classifies new accessibility and console fingerprints", async () => {
    ...
    try {
      ...
      const statesFor = (category: "a11y" | "console") =>
        compared.manifest.targets[0]!.findings.filter((finding) => finding.category === category)
          ...
          .sort();
      expect(statesFor("a11y")).toEqual(["new", "resolved", "unchanged", "unchanged", "unchanged"]);
