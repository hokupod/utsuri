...
4. Confirm that none of the five `0.3.5` versions already exists. If one exists, reconcile its registry integrity against the approved candidate before continuing.
5. Create annotated tag `v0.3.5` at the still-current exact `main` commit and push only that tag.
