...
The publisher is `hokupod`, the npm maintainer identity is `hokupod-npm`, npm publication uses GitHub Actions trusted publishing, and the SPDX license is `AGPL-3.0-or-later`. Release validation rejects different or unresolved values.
